...
namespace UnderWhere
{
    ...
    static class HarmonyPatching
    {
        ...
        {
            var harmony = new Harmony("{rimworld.malistaticy.underwhere}");
    ...
    //    }
    //}
    public class ApparelUtility_Patch
    {
        [HarmonyPatch(typeof(ApparelUtility), "HasPartsToWear")]
        public class HasPartsToWear
        {
            [HarmonyPrefix]
            public static bool Prefix(Pawn p, ThingDef apparel)
            {
                // Check if allowed
                var forMales = apparel.HasComp(typeof(CompMaleOnly));
                var forFemales = apparel.HasComp(typeof(CompFemaleOnly));


                if (forFemales && p.gender != Gender.Female)
                {
                    return false;
                }
                if (forMales && p.gender != Gender.Male)
                {
                    return false;
                }
                return true;
